...
class ReportPortalAgentDebug extends AbstractAgent {
  /**
  ...
  /**
   *
   * @returns {Promise<T>} ReportPortal Unique ID
  ...
  __finishTestItem(itemInfo, allowEmptyItemInfo) {
    this.requestsProcessList.push(() => {
      console.log(
        `Finish test item ${this.agentContex.state.getCurrentState()}`,
      );
  ...
  sendLogItemRequest(itemInfo, fileObject, parrentId, callback) {
    this.requestsProcessList.push(() => {
      ...
      if (
        ...
        if (callback) {
          console.log('Invoke with callback for log item');
...
exports.ReportPortalAgentDebug = ReportPortalAgentDebug;
